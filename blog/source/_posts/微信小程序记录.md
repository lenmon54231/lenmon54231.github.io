...
微信小程序提供的rich-text仅适合简单富文本业务，复杂业务如：视频播放等无法比较好的展示出来，所以需要有额外的富文本解析组件来实现，如：mp-html等，
但是mp-html对视频解析是有问题的，所以引入了 wxParse组件库

使用
```js
<wxParse nodes="{{ currentArticleInfo.content }}" />
```


wxParse组件代码位于：根目录/example-code/微信小程序使用/wxParse

组件支持markdown和html两种解析模式
...
```js
shopRequest("checkout/confirm", {
    data: orderData,
  }).then((res) => {
      usePayment(res.provider., res.order_id)
            .then(() => {
              console.log('支付成功')
    })
})
...
export default function usePayment(provider, order_id) {
   return new Promise((resolve, reject) => {
    shopRequest(`orders/${order_id}/pay`)
      .then((res) => {
        wx.requestPayment({
          // provider: "wxpay",
          provider: provider,
...
实现步骤：
> 1. checkout/confirm传入支付数据，生成订单信息；
> 2. 获取到生成的订单信息后，填入支付方式(provider)和订单id(order_id)，调用usePayment；
> 3. usePayment通过order_id接口查询到wx.requestPayment支付方法所需要的数据，则可以调起官方微信支付功能

tips：

> provider：支付方式，这是与后端约定的一个自定义属性，非支付必选属性，此处wxpay为微信支付的意思


### 微信判断蓝牙权限

微信蓝牙权限分为3种：1.手机蓝牙权限；2.手机授予微信蓝牙权限；3.微信授予小程序蓝牙权限。

那么怎么精准的提示用户哪些权限没有开启呢？

```js
// ----------------------首先判断微信授予小程序蓝牙权限---------------------------
const res = await askForAuth("scope.bluetooth", {
    content: "定位功能需要使用“蓝牙”权限",
  });
  console.log("res:askForAuth ", res);

  if (!res) {
    showToastWithoutIcon("请确保小程序蓝牙权限已授权！", {
      duration: 5000,
    });
    sendSelfTrackEvent({
      type: "error",
      data: {
        bluetooth: "小程序未授权蓝牙权限",
      },
    });
    return;
}
...
wx.openBluetoothAdapter({
    ...
    fail: (err) => {
      ...
      if (err.errCode === 10001) {
        showToastWithoutIcon("请确保手机已经授权微信的蓝牙权限", {
          duration: 5000,
        });
        sendSelfTrackEvent({
          type: "error",
          data: {
            bluetooth: "手机未授权微信蓝牙权限",
          },
        });
      } else {
        // 未知蓝牙错误，需要查询微信开发者文档
        showToastWithoutIcon("蓝牙工作异常！请稍后再试~", {
          duration: 5000,
        });
        sendSelfTrackEvent({
          type: "error",
          data: {
            bluetooth: "蓝牙错误码：" + String(err.errCode),
          },
        });
      }
    },
    success: (success) => {
      ...
      // --------------------最后判断手机蓝牙权限------------------------
      //必须要手机蓝牙开启并且授予微信蓝牙权限，bluetoothEnabled才为true
      if (!systemSetting.bluetoothEnabled) {
        showToastWithoutIcon("请确保手机蓝牙已打开！", {
          duration: 5000,
        });
        sendSelfTrackEvent({
          type: "error",
      ...
      this.emit("isBluetoothReady");
      wx.onBluetoothAdapterStateChange((res) => {
        const { available } = res;
        console.log("available: ", available);
        if (!available) {
          // 手机未打开蓝牙权限
          showToastWithoutIcon("请打开手机蓝牙权限");
          sendSelfTrackEvent({
            type: "error",
            data: {
              bluetooth: "手机未打开蓝牙权限",
...
### scroll-view中的元素无法通过z-index覆盖scroll-view外的元素

```wxml
<view class="container">
  <view style="height: 40vh; background-color: red; width: 100vw"></view>
  <scroll-view scroll-y="{{true}}" class="scroll-view-container">
    <!-- 通过root-portal设置元素层级有效 -->
    <root-portal enable="{{true}}" style="width: 100vw; height: 220rpx">
      <view class="inner">我是弹出层</view>
    </root-portal>

    <!-- 通过z-index设置元素层级无效 -->
    <!-- <view class="inner-no-portal">我是弹出层</view> -->
  </scroll-view>
